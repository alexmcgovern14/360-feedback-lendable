...
**Demo without a database:** Set **USE_JSON_DATA=true** in Vercel (and leave `DATABASE_URL` unset or remove it). The app will serve from static `data/seed.json` and persist runtime prototype data to **Vercel Blob** as JSON artifacts:\n+- Reviewer chat transcripts (state + timestamped snapshots)\n+- Structured per-review JSON\n+- Combined step1/2/3 JSON\n+- Manager edits + finalisation status\n+\n+To enable persistence, connect a Blob store to the Vercel project (Storage → Blob) so `BLOB_READ_WRITE_TOKEN` is available. Locally, run `vercel env pull` to populate `.env.local`.
